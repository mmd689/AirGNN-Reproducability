...
def nettack_run(dataset_name, run_k, n_perturbations_candidates):
    ...
    for target_node in node_list:
        ...
        s1, s2 = adv_test(target_node, data, data.adj[target_node].nonzero()[1].tolist(), run_k)
        target_score_list.append(s1.item())
        rest_score_list.append(s2.item())

        print("=========Attacked Node: {:d}, n_perturbations: {:.2f}=========".format(target_node, perturbation))
        print(args.model, args.lambda_amp)


    print("Score on 40 target nodes:", np.mean(target_score_list))
    print("Score on the rest of the nodes", np.mean(rest_score_list))
...
def adv_test(key_node_index, attacked_dpr_data, neighbor_lst, run_k):
    ...
    node_score = score[key_node_index]
    rest_mean = (torch.sum(score).item() - node_score) / (score.size()[0] - 1)
    # print("single_target_predict\n:", logits.argmax(1)[key_node_index].item(), data.y[key_node_index].item(), target_node_acc)
    return node_score, rest_mean
